...
// toolComputeIndicators — the registry-driven indicator engine, screen mode
// (ticket 01). Pure DB read over daily_prices: indicators are computed on
// demand from stored rows, nothing is precomputed or persisted, and the MCP
// request path never touches the nodriver sidecar (Heroku H12 constraint,
// ADR-0009). The registry list in the description is generated from
...
var toolComputeIndicators = mcpgo.NewTool("compute_indicators",
	mcpgo.WithDescription(fmt.Sprintf("Latest registered-indicator values for one or more tickers, computed on demand from stored Daily Price rows. Pure DB read — no upstream call. mode is explicit: screen (the only mode today) returns one row per ticker carrying the latest value per requested indicator; series mode (full per-day arrays for exactly one ticker) is not implemented yet. tickers takes up to 50 codes — an over-cap call is rejected, never silently truncated. indicators takes registry specs: a bare name uses the entry's default periods as listed below, and a name carrying periods overrides them — \"sma:50\" for a single-period entry, \"ma_distance:10:30\" for a fast-then-slow pair; periods are 2..400, and an entry with no period (macd, obv) rejects one. Registry — %s. A value whose stored history is shorter than the indicator's warm-up (a 200-period MA over 42 stored rows) is null and keyed in that row's insufficient array — never computed on a short window, which would read as fully warmed when it is not. An indicator reading high, low, or volume computes over the rows that stored that column (never over a zero substituted for a missing one), so a day without volume leaves it insufficient too. window is the number of stored trading days loaded per ticker (default 250, max 500); history_rows reports how many usable closes the values rest on, and required_rows the binding warm-up bar. as_of pins the anchor trading day (default: the latest stored market-wide trading day, so every row in a multi-ticker call is same-day). Rows follow the requested ticker order. Staleness shows in data_stale / last_good_date.", indicator.Catalog())),
	mcpgo.WithString("mode", mcpgo.Description("\"screen\" (default) — latest values, one row per ticker. \"series\" is not implemented yet.")),
	mcpgo.WithArray("tickers", mcpgo.WithStringItems(), mcpgo.Description("Ticker codes (e.g. BBRI or BBRI.JK). Up to 50 per call; over-cap is rejected."), mcpgo.Required()),
