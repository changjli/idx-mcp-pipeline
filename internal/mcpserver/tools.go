...
var toolScreenStocks = mcpgo.NewTool("screen_stocks",
	mcpgo.WithDescription(fmt.Sprintf("Whole-universe funnel screener over stored Daily Price rows: one call turns the persisted market into a ranked shortlist. Pure DB read — no upstream call. The funnel runs in stages and every response reports them. Stage 1, the SQL hard filters, keeps a ticker that (a) has a stored row ON the anchor trading day — a halted or delisted name's latest row is older, so it drops out, (b) traded at least min_value rupiah on that day, and (c) has no suspension-related event (SPT suspend, UPT resume, or UMA warning) in the last suspension_window_days trading days. Stage 2 computes the requested indicators for the survivors only — the universe's price history is never loaded to answer a screen that discards it. Stage 3 applies the structural filter set: threshold comparisons on single registered indicator values, AND-combined, so \"price above MA20\" is ma_distance > 0 and the sideways-breakout-zone shape is that plus a range_position band. Omitting filters runs the shipped default set (%s); passing [] runs no structural filters; a non-empty list replaces the default set. A filter indicator is computed even when it was not requested as a column, and a row whose filter value is null is dropped rather than assumed to pass. The funnel block reports universe (tickers with a row inside the window), after_value (survivors of the hard filters), and after_structure (survivors of the filters), so a filter set that keeps everything or nothing is visible instead of guessed at. Each row carries the ticker, its anchor-day value and close, and one column per computed indicator; a value is null whenever the stored history is shorter than that indicator's warm-up, and the key is listed in the row's insufficient array — never a short-window number that would read as fully warmed, and never a zero standing in for \"no value\". Default columns: %s. Every row also carries foreign_net: the stored foreign net (Σ f_nval from the broker footer totals, which covers brokers below IPOT's top-10) over the last flow_window_days stored trading days, joined for the survivors only. That population is anomaly-gated and therefore sparse by design, so the column is always accompanied by a per-row coverage block keyed by column name — coverage.foreign_net.observed says whether anything was stored for that ticker in the window and coverage.foreign_net.days how many of the window's trading days it rests on — and an uncovered row carries a null foreign_net, never a zero standing in for \"no data\". The response echoes flow_window_days and flow_days_in_window (the trading days the window actually spans), so a row's day count reads against its denominator. sort names the ranking key: %q (the anchor day's transaction value, the default), \"foreign_net\", or any computed indicator key; order is \"desc\" (default) or \"asc\". A row whose sort-key value is unobserved ranks last in either direction; ties break by anchor-day value descending, then ticker ascending, so identical inputs always produce identical order. window is the lookback in stored trading days — the population the funnel counts and the history each survivor's indicators rest on (default 250, max 500); as_of pins the anchor trading day (default: the latest stored market-wide trading day, so every row is same-day). limit caps the returned rows (default 30, max 50, an over-cap call is rejected, never silently truncated) and total_matches reports how many tickers passed every filter before that cap — the difference is the cut. Staleness shows in data_stale / last_good_date.",
		usecase.ScreenerDefaultFiltersDoc(), strings.Join(usecase.ScreenerDefaultIndicators, ", "), "value")),
	...
	mcpgo.WithNumber("flow_window_days", mcpgo.Description("Stored trading days the broker-flow column reaches back over, ending on the anchor day. Default 20, max 60 — broker rows are retained 90 calendar days, so a longer window can never be observed and is rejected rather than returned mostly empty. Independent of `window`.")),
	...
	mcpgo.WithArray("filters", mcpgo.Items(map[string]any{
		"type": "object",
		"properties": map[string]any{
			"indicator": map[string]any{
				"type":        "string",
				"description": "A registry spec, e.g. \"ma_distance:20:50\", or a bare name for the entry's default periods.",
			},
			"op": map[string]any{
				"type":        "string",
				"enum":        []string{"lt", "gt", "lte", "gte", "between"},
				"description": "Threshold comparison applied to the indicator's latest value.",
			},
			"value": map[string]any{
				"description": "One number for lt/gt/lte/gte; [low, high] for between, inclusive at both ends.",
		...
		"required": []string{"indicator", "op", "value"},
	}), mcpgo.Description("Structural filters, AND-combined, each a threshold on ONE registered indicator's latest value. Omit to run the shipped default set (listed above); pass [] to run no structural filters at all; a non-empty list replaces the default set rather than adding to it. Filter indicators join the columns even when not requested, so a row shows the number it was judged on and sort may name it. A row whose filter value is null — the indicator below its warm-up — is dropped, never assumed to pass. Direct indicator-to-indicator comparison belongs in a derived indicator (ma_distance for close-vs-MA), not here; crossovers and streaks are out of scope and belong to compute_indicators series mode.")),
