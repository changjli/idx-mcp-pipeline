...
func TestGraphWaveMembershipComplete(t *testing.T) {
	...
	want := []string{TypeStockSummary, TypeAnnouncements, TypeRSS, TypeCorporateActions, TypeCleanup}
...
func TestGraphSelfHealSetExact(t *testing.T) {
	...
	want := map[string]bool{TypeStockSummary: true, TypeAnnouncements: true, TypeRSS: true, TypeCorporateActions: true}
	if len(got) != len(want) {
